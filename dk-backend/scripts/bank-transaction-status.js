...
const transferResponsePath = path.join(outputDir, "fund-transfer-response.json");
...
const statusUrl =
  ...
  `${process.env.BANK_API_BASE_URL}/v1/transaction/status`;
...
const transferData = await readJsonFile(transferResponsePath);
const transactionId =
  args[0] ||
  transferData?.response_data?.transaction_id ||
  transferData?.response_data?.txn_status_id ||
  transferData?.transaction_id ||
  transferData?.txn_status_id;
const beneficiaryAccount =
  args[1] ||
  process.env.BANK_TEST_BENEFICIARY_ACCOUNT;

if (!transactionId || !beneficiaryAccount) {
  await fail("Missing transaction id or beneficiary account.", {
    usage: "npm run bank:status -- <transaction_id> <beneficiary_account>",
    defaults: [
      "transaction id from tmp/bank-api/fund-transfer-response.json",
      "beneficiary account from BANK_TEST_BENEFICIARY_ACCOUNT",
    ],
  });
}

if (!/^\d{12}$/.test(beneficiaryAccount)) {
  await fail("Beneficiary account must be exactly 12 digits.", {
    providedPreview: redact(beneficiaryAccount, 4, 4),
    fix: "Pass the real account explicitly or set BANK_TEST_BENEFICIARY_ACCOUNT in .env",
  });
}
...
await writeJson("transaction-status-request-preview.json", {
  ...statusBody,
  bene_account_number: redact(statusBody.bene_account_number, 4, 4),
});
...
if (!authResponse.ok || !accessToken) {
  await writeJson("transaction-status-auth-error.json", authData);
  await fail("Authorization token request did not return access_token", {
    httpStatus: authResponse.status,
    savedTo: "tmp/bank-api/transaction-status-auth-error.json",
  });
}
...
if (!signKeyResponse.ok || !signKey) {
  await writeJson("transaction-status-sign-key-error.json", signKeyData);
  await fail("Sign key request did not return a key", {
    httpStatus: signKeyResponse.status,
    savedTo: "tmp/bank-api/transaction-status-sign-key-error.json",
  });
}

console.log(`   OK key=${redact(signKey.replaceAll("\n", ""), 18, 12)}`);
console.log("3. Checking transaction status...");
...
const statusResponse = await fetch(statusUrl, {
  ...
  headers: {
    ...
    Authorization: `Bearer ${accessToken}`,
    ...gatewayHeaders,
    ...buildDkSignatureHeaders(statusBody, signKey),
  },
  body: canonicalJson(statusBody),
});
const statusData = await readJson(statusResponse);
await writeJson("transaction-status-response.json", statusData);

if (!statusResponse.ok || statusData.response_code && statusData.response_code !== "0000") {
  await fail("Transaction status check failed", {
    httpStatus: statusResponse.status,
    response_code: statusData.response_code,
    response_detail:
      statusData.response_detail ||
      statusData.response_message ||
      statusData.response_description ||
      statusData.message,
    savedTo: "tmp/bank-api/transaction-status-response.json",
  });
}

console.log(`   OK response_code=${statusData.response_code || "HTTP_200"}`);
console.log("Done. Transaction status response saved under tmp/bank-api/transaction-status-response.json.");
